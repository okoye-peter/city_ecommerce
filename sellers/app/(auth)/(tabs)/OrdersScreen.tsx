...
const OrdersScreen = () => {
    ...
    const filteredOrders = OrderItems.filter(order => {
        ...
        if (activeTab === 1) return ['pending', 'accepted', 'ready_for_pickup', 'picked_up'].includes(order.status); // Ongoing
        if (activeTab === 2) return order.status === 'delivered'; // Delivered
        if (activeTab === 3) return order.status === 'cancel'; // Declined
        return true;
    });
